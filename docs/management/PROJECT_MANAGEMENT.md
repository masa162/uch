...
### 2025年8月20日: VPS本番環境のDocker設定最適化 (TSK-085)

**概要**:
VPS上の本番環境におけるDockerコンテナの運用を、よりセキュアで再現性の高い状態に改善しました。

**主な変更点**:
- **`docker-compose.prod.yml` の最適化**:
    - データベースサービスを削除し、外部DBとの接続設定に変更。
    - アプリケーションサービスを `127.0.0.1:3000` にバインドし、セキュリティを向上。
    - 環境変数を `.env.production` から直接設定に移行。
    - Node.jsベースのヘルスチェックを実装。
- **GitHub Actionsワークフローの最適化**:
    - コミットSHAを使用したイメージタグの動的更新を実装。
    - デプロイ時に `docker-compose.prod.yml` のイメージタグを自動更新。
    - 再現性のあるデプロイフローを確立。

**効果**:
- PostgreSQLコンテナの外部公開を停止し、セキュリティが大幅に向上しました。
- `:latest` タグから具体的なコミットSHAタグへの移行により、デプロイの再現性が確保されました。
- 本番環境に最適化されたDocker Compose設定が適用され、運用がより堅牢になりました。
